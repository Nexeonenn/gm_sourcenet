...
NET_MESSAGES = {
	...
	[net_SetConVar] = { -- 5
		__tostring = function(self)
			...
			for name, value in pairs(self.cvars) do
				if values then
					values = string.format("%s, %s = %s", values, name, value)
				else
					values = string.format("%s = %s", name, value)
				end
			end
		...
		__index = {
			...
			ReadFromBuffer = function(self, buffer)
				self.count = buffer:ReadByte()
				self.cvars = {}
				for i = 1, self.count do
					local cvarname = buffer:ReadString()
					local cvarvalue = buffer:ReadString()
					self.cvars[cvarname] = cvarvalue
				end

				self.initialized = true
			end,
			-- WriteToBuffer doesn't keep the order of convars from a previous ReadFromBuffer
			WriteToBuffer = function(self, buffer)
				if not self.initialized then
					return false
				end
				...
				buffer:WriteUInt(net_StringCmd, NET_MESSAGE_BITS)
				buffer:WriteByte(self.count)

				for name, value in pair(self.cvars) do
	...
	[net_SignonState] = { -- 6
		__tostring = function(self)
			...
			return string.format("net_SignonState: state = %i, servercount = %i", self.state, self.servercount)
		end,
		__index = {
			GetType = function()
				return net_SignonState
	...
	SVC = {
		...
		[svc_SetPause] = { -- 11
			__tostring = function(self)
				...
				return "svc_SetPause: " .. self.paused
			end,
			__index = {
				GetType = function()
					return svc_SetPause
		...
		[svc_GetCvarValue] = { -- 31
			__tostring = function(self)
				...
				return "svc_GetCvarValue: cvarname = " .. cvarname
			end,
			__index = {
				GetType = function()
					return svc_GetCvarValue
		...
		[svc_GMod_ServerToClient] = { -- 33
			...
			__index = {
				...
				ReadFromBuffer = function(self, buffer)
					...
					elseif self.msgtype == 4 then
						self.length = buffer:ReadUInt(16)
						self.data = buffer:ReadBytes(self.length)
					end
				...
				WriteToBuffer = function(self, buffer)
					...
					elseif self.msgtype == 4 then
						buffer:WriteUInt(self.length, 16)
						buffer:WriteBytes(self.data)
					end
